...
let PACER = {
  ...
  normalizeDashes: (text) => {
    ...
    let figure_dash = '‒';
    let horizontal_bar = '―';
    return text.replace(new RegExp(`[${en_dash}${em_dash}${hyphen}${non_breaking_hyphen}${figure_dash}${horizontal_bar}]+`), normal_dash);
  },
  ...
  isAttachmentMenuPage: function (url, document) {
    ...
    let pageCheck = PACER.isDocumentUrl(url) && (!!buttonText || !!bigFile || !!topNote || !!bottomNote);
    return !!pageCheck;
  },
  ...
  isSingleDocumentPage: function (url, document) {
    ...
    let pageCheck =
      (PACER.isDocumentUrl(url) && (hasAudioReceipt || hasImageReceipt || hasTranscriptReceipt) && lastInput === 'View Document') ||
  ...
  parseGoDLSFunction: function (goDLS_string) {
    ...
    let r = {};
    if (goDlsDistrict) {
      [, r.hyperlink, r.de_caseid, r.de_seqno, r.got_receipt, r.pdf_header, r.pdf_toggle_possible, r.magic_num, r.hdr, r.psf_report] =
        goDlsDistrict;
    } else {
      [
        ,
