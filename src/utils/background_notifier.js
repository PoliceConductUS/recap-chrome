...
const showNotification = function (title, message, cb) {
  ...
  if (/Safari/.test(navigator.userAgent) && !/Chrome|Chromium/.test(navigator.userAgent)) return;
  ...
  switch (action) {
    ...
    case 'showUpload':
      chrome.storage.local.get('options', (items) => {
        if (items && items.options.show_notifications) showNotification(title, message);
        sendResponse({ status: 'success ' });
      });
      break;
    default:
